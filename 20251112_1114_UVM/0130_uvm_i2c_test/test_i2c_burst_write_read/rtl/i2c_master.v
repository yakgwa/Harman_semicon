...
module I2C_Clk_Div #(
    ...
    always @(posedge clk or negedge reset_n) begin
        if(!reset_n) begin
            ...
            // tick_sample 발생
            tick_sample <= (counter == CLK2);

            // SCL 생성
            if(counter < CLK0) gen_scl <= 0;
            else if(counter < CLK2) gen_scl <= 1;
            else gen_scl <= 0;

            // counter 증가
            if(counter == CLK3-1) counter <= 0;
...
//             tick_sample <= 0;
//         end else if(scl_en) begin
//             if(counter_reg == CLK2) begin
//             //if(counter_reg == CLK3-2) begin
//                 tick_sample <= 1;
//             end else begin
//                 tick_sample <= 0;
//             end
...
//             // SCL 생성
//             if(counter_reg < CLK0) gen_scl <= 0;
//             else if(counter_reg < CLK2) gen_scl <= 1;
//             else gen_scl <= 0;

//             // counter 증가
//             if(counter_reg == CLK3-1) counter_reg <= 0;
