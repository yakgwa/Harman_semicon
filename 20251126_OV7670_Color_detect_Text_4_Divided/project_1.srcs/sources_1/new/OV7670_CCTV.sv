...
module OV7670_CCTV (
    ...
    logic [9:0] adjusted_x_pixel;
    logic [9:0] adjusted_y_pixel;

    assign adjusted_x_pixel = (x_pixel >= 10'd320) ? (x_pixel - 10'd320) : x_pixel;  
    assign adjusted_y_pixel = (y_pixel >= 10'd240) ? (y_pixel - 10'd240) : y_pixel;

    logic overlay_img_on;      
    logic [16:0] overlay_addr;  
    logic [15:0] overlay_data_in; 
    
    logic [3:0] overlay_r_port, overlay_g_port, overlay_b_port; 
    logic [11:0] overlay_rgb444; 
    logic chroma_key_active; 

    localparam [11:0] CHROMA_KEY_COLOR = 12'hFFF; 

    assign overlay_img_on = DE; 

    assign overlay_addr = (adjusted_y_pixel * 10'd320) + adjusted_x_pixel; 
    
    assign overlay_r_port = overlay_data_in[15:12]; 
    assign overlay_g_port = overlay_data_in[10:7];  
    assign overlay_b_port = overlay_data_in[4:1];   
    assign overlay_rgb444 = {overlay_r_port, overlay_g_port, overlay_b_port};

    assign chroma_key_active = (overlay_rgb444 == CHROMA_KEY_COLOR);

    logic border_on;
    localparam [3:0] ORANGE_R = 4'b1111; // High Red
    localparam [3:0] ORANGE_G = 4'b1000; // Medium Green (for Orange hue)
    localparam [3:0] ORANGE_B = 4'b0000; // Low Blue

    assign border_on = DE & 
                       (((x_pixel >= 10'd319) && (x_pixel <= 10'd320)) || // Vertical center line
                        ((y_pixel >= 10'd239) && (y_pixel <= 10'd240))); // Horizontal center line

    assign xclk = sys_clk;

    assign r_port = (text_on) ? text_r : 
                    (border_on) ? ORANGE_R :
                    (overlay_img_on && !chroma_key_active) ? overlay_r_port :
                    w_r_port;
    assign g_port = (text_on) ? text_g : 
                    (border_on) ? ORANGE_G :
                    (overlay_img_on && !chroma_key_active) ? overlay_g_port :
                    w_g_port;
    assign b_port = (text_on) ? text_b : 
                    (border_on) ? ORANGE_B :
                    (overlay_img_on && !chroma_key_active) ? overlay_b_port :
                    w_b_port;

    overlay_image_rom U_OVERLAY_ROM (
        .addr(overlay_addr),
        .data(overlay_data_in)
    );
...
//  );
    ...
    logic [3:0] w_commend;
    logic       w_frame_end;
    logic [3:0] w_blue_frame_count;
    logic [3:0] w_red_frame_count;
    ...
    color_counter U_COLOR_COUNTER (
        .clk(sys_clk),
        .reset(reset),
        .i_blue_detect(w_blue_detect),
        .i_red_detect(w_red_detect),
        .i_frame_end(w_frame_end), 
        .o_blue_count(w_blue_frame_count), 
        .o_red_count(w_red_frame_count)    
    );

    always_comb begin
        if (w_blue_detect)
            w_commend = 4'b0001; // BLUE_DETECT
        else if (w_red_detect)
            w_commend = 4'b0010; // RED_DETECT
        else
            w_commend = 4'b0000; // GAME_START
    end
    ...
    text_display U_TEXT (
        ...
        //.reset(reset),
        .DE(DE),
        .blue_count(w_blue_frame_count),
        .red_count(w_red_frame_count),
        .i_commend(w_commend),
        .x_pixel(x_pixel),
        .y_pixel(y_pixel),
        .o_red(text_r),
...
//  ... filter modules ...
    ...
    ImgMemReader U_IMG_Reader (
        .DE(DE),
        .x_pixel(adjusted_x_pixel), 
        .y_pixel(adjusted_y_pixel),
        .addr(rAddr),
        .imgData(rData),
...
module overlay_image_rom (
    input  logic [16:0] addr, // 17-bit address for 320*240=76800 locations
    output logic [15:0] data  // 16-bit RGB565 data
);

    localparam ROM_SIZE = 640 * 480; 
    logic [15:0] rom[0:ROM_SIZE-1];

    initial begin
        // The memory file 'overlay.mem' is assumed to contain 16-bit RGB565 data
        // For chroma keying, a specific color (e.g., pure black) can be used as transparency.
        $readmemh("Test2.mem", rom); 
    end
...
//    localparam [3:0] YELLOW_R = 4'b1111;
//    localparam [3:0] YELLOW_G = 4'b1111;
//    localparam [3:0] YELLOW_B = 4'b0000;

//    assign rom_on = DE & 
//                    (((x_pixel >= 10'd310) && (x_pixel <= 10'd329)) || 
//                     ((y_pixel >= 10'd230) && (y_pixel <= 10'd249))); 

//    assign xclk = sys_clk;

//    assign r_port = (text_on) ? text_r : (rom_on ? YELLOW_R : w_r_port);
//    assign g_port = (text_on) ? text_g : (rom_on ? YELLOW_G : w_g_port);
//    assign b_port = (text_on) ? text_b : (rom_on ? YELLOW_B : w_b_port);
...
//    logic rom_on;
//    logic [9:0] rom_x_coor; 
//    logic [9:0] rom_y_coor; 
//    logic [16:0] rom_addr;  
//    logic [15:0] rom_data_in; 
    
//    logic [3:0] rom_r_port, rom_g_port, rom_b_port;

//    assign rom_on = DE & 
//                    (((x_pixel >= 10'd310) && (x_pixel <= 10'd329)) || 
//                     ((y_pixel >= 10'd230) && (y_pixel <= 10'd249))); 
                     
//    assign rom_x_coor = (x_pixel > 10'd160) ? (x_pixel - 10'd160) : 10'd0; 
    
//    assign rom_y_coor = (y_pixel > 10'd120) ? (y_pixel - 10'd120) : 10'd0; 

//    assign rom_addr = (rom_y_coor * 10'd320) + rom_x_coor; 
    
//    assign rom_r_port = rom_data_in[15:12]; 
//    assign rom_g_port = rom_data_in[10:7];  
//    assign rom_b_port = rom_data_in[4:1];   

//    assign xclk = sys_clk;
    
//    assign r_port = (text_on) ? text_r : (rom_on ? rom_r_port : w_r_port);
//    assign g_port = (text_on) ? text_g : (rom_on ? rom_g_port : w_g_port);
//    assign b_port = (text_on) ? text_b : (rom_on ? rom_b_port : w_b_port);
...
module Laplacian_Filter #(
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            // 3. Valid Pipeline ������Ʈ (x_coor >= 2, y_coor >= 2�� �� ��ȿ)
