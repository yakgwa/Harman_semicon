...
module red_tracker_manual (
    ...
    logic is_red;
    assign is_red = (r_val > 5'd20) && (g_val < 6'd22) && (b_val < 5'd22);
    ...
    // Ï¢åÌëú Í≥ÑÏÇ∞?ö© Î≥??àò (Threshold Logic)
    localparam int THRESHOLD = 10; // 10?îΩ?? ?ù¥?ïò?äî Î¨¥Ïãú (?öù?öù ?ÅäÍ∏∞Îäî ?ö®Í≥?)
    int raw_cx, raw_cy;
    int diff_x, diff_y;
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (vsync_start) begin
                ...
                // 4. ÏµúÏ¢Ö Í≤∞Í≥º ?óÖ?ç∞?ù¥?ä∏ (?ç∞?ìúÏ°? Î°úÏßÅ ?†Å?ö©)
                for (i = 0; i < 16; i = i + 1) begin
                    // Í∞êÏ? ?ÉÅ?Éú ?óÖ?ç∞?ù¥?ä∏ (Register)
                    ...
                    if (t_detected[i]) begin
                        ...
                        // [Ï§ëÏöî ?àò?†ï] Í∞ïÏ†ú ?òïÎ≥??ôò(int')?ùÑ ?Üµ?ï¥ Signed ?ó∞?Ç∞ Î≥¥Ïû•
                        // aim_x_all?ù¥ unsigned?ùº?Ñú Î∞úÏÉù?ïò?äî ?ò§Î≤ÑÌîåÎ°úÏö∞ Î∞©Ï?
                        ...
                        // [?ùî?ì§Î¶? Î∞©Ï?]
                        // 1. ?ù¥?†Ñ?óê Í∞êÏ??êòÏß? ?ïä?ïò?çò Î¨ºÏ≤¥Î©?(0->1) Ï¶âÏãú ?ä§?ÉÖ
                        if (aim_detected_all[i] == 0) begin
                            aim_x_all[i] <= raw_cx;
                            aim_y_all[i] <= raw_cy;
                        end 
                        // 2. ?ù¥ÎØ? Í∞êÏ?Ï§ëÏù∏ Î¨ºÏ≤¥Î©?(1->1) Î≥??ôî?üâ?ù¥ ?Å¥ ?ïåÎß? Í∞±Ïã†
                        else begin
